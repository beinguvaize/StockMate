...
const Inventory = () => {
    ...
    useEffect(() => {
        if (showAddModal || showHistoryModal) {
            document.body.style.overflow = 'hidden';
        } else {
            document.body.style.overflow = 'unset';
        }
        return () => { document.body.style.overflow = 'unset'; };
    }, [showAddModal, showHistoryModal]);
                ...
                <div className="modal-overlay">
                    <div className="glass-modal !max-w-2xl">
                        <div className="flex justify-between items-start mb-3 border-b border-black/5 pb-3">
                            <div>
                                <h1 className="text-xl font-black text-ink-primary tracking-tighter uppercase leading-none mb-1">
                                    {editingProduct ? 'EDIT ASSET' : 'NEW ASSET'}.
                                </h1>
                                <p className="text-[9px] font-black text-ink-secondary uppercase tracking-[0.2em] opacity-60">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-3 mt-2">
                            <div className="grid grid-cols-1 md:grid-cols-6 gap-3">
                                ...
                                <div className="md:col-span-4">
                                    <label className="block text-[9px] font-black uppercase tracking-widest text-ink-secondary opacity-50 mb-0.5 ml-1">Asset Name</label>
                                    <input required type="text" className="w-full bg-canvas border-none rounded-lg p-2.5 font-black text-lg text-ink-primary outline-none focus:ring-2 focus:ring-accent-signature/20 transition-all uppercase" placeholder="PRODUCT NAME..." value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
                                </div>
                                <div className="md:col-span-2">
                                    <label className="block text-[9px] font-black uppercase tracking-widest text-ink-secondary opacity-50 mb-0.5 ml-1">Category</label>
                                    <select className="w-full bg-canvas border-none rounded-lg p-2.5 font-black text-xs text-ink-primary outline-none focus:ring-2 focus:ring-accent-signature/20 transition-all uppercase appearance-none" value={formData.category} onChange={e => setFormData({ ...formData, category: e.target.value })}>
                                ...
                                <div className="md:col-span-2">
                                    <label className="block text-[9px] font-black uppercase tracking-widest text-ink-secondary opacity-50 mb-0.5 ml-1">SKU Barcode</label>
                                    <input required type="text" className="w-full bg-canvas border-none rounded-lg p-2 font-black text-xs text-ink-primary outline-none focus:ring-2 focus:ring-accent-signature/20 transition-all uppercase" placeholder="SKU-XXXX" value={formData.sku} onChange={e => setFormData({ ...formData, sku: e.target.value })} />
                                </div>
                                <div className="md:col-span-1">
                                    <label className="block text-[9px] font-black uppercase tracking-widest text-ink-secondary opacity-50 mb-0.5 ml-1">Cost</label>
                                    <input required type="number" step="0.01" className="w-full bg-canvas border-none rounded-lg p-2 font-black text-xs text-ink-primary outline-none focus:ring-2 focus:ring-accent-signature/20 transition-all" value={formData.costPrice} onChange={e => setFormData({ ...formData, costPrice: e.target.value })} />
                                </div>
                                <div className="md:col-span-1">
                                    <label className="block text-[9px] font-black uppercase tracking-widest text-ink-secondary opacity-50 mb-0.5 ml-1">Retail</label>
                                    <input required type="number" step="0.01" className="w-full bg-canvas border-none rounded-lg p-2 font-black text-xs text-ink-primary outline-none focus:ring-2 focus:ring-accent-signature/20 transition-all" value={formData.sellingPrice} onChange={e => setFormData({ ...formData, sellingPrice: e.target.value })} />
                                </div>
                                <div className="md:col-span-2">
                                    <label className="block text-[9px] font-black uppercase tracking-widest text-ink-secondary opacity-50 mb-0.5 ml-1">Unit</label>
                                    <select className="w-full bg-canvas border-none rounded-lg p-2 font-black text-xs text-ink-primary outline-none focus:ring-2 focus:ring-accent-signature/20 transition-all uppercase appearance-none" value={formData.unit} onChange={e => setFormData({ ...formData, unit: e.target.value })}>
                                ...
                                <div className="md:col-span-3">
                                    <label className="block text-[9px] font-black uppercase tracking-widest text-ink-secondary opacity-50 mb-0.5 ml-1">Initial Stock</label>
                                    <input required type="number" className="w-full bg-canvas border-none rounded-lg p-2 font-black text-xs text-ink-primary outline-none focus:ring-2 focus:ring-accent-signature/20 transition-all" value={formData.stock} onChange={e => setFormData({ ...formData, stock: e.target.value })} disabled={!!editingProduct} />
                                </div>
                                <div className="md:col-span-3">
                                    <label className="block text-[9px] font-black uppercase tracking-widest text-ink-secondary opacity-50 mb-0.5 ml-1">Low Stock Alert</label>
                                    <input type="number" className="w-full bg-canvas border-none rounded-lg p-2 font-black text-xs text-ink-primary outline-none focus:ring-2 focus:ring-accent-signature/20 transition-all" value={formData.lowStockThreshold} onChange={e => setFormData({ ...formData, lowStockThreshold: e.target.value })} />
                                </div>

                                {/* Line 4: Image Preview (3) + Tax Slabs (3) */}
                                <div className="md:col-span-3">
                                    <label className="block text-[9px] font-black uppercase tracking-widest text-ink-secondary opacity-50 mb-0.5 ml-1">Visual Asset</label>
                                    <div 
                                ...
                                <div className="md:col-span-3">
                                    <label className="block text-[9px] font-black uppercase tracking-widest text-ink-secondary opacity-50 mb-0.5 ml-1">Taxation Compliance</label>
                                    <div className="grid grid-cols-3 gap-1.5 h-16 content-start">
                                        {TAX_SLABS.map(slab => (
                                            <button
                                                key={slab.label}
                                                type="button"
                                                className={`p-1 rounded-lg text-[8px] font-black uppercase tracking-tighter transition-all border flex flex-col items-center justify-center gap-0 ${
                                            ...
                                            >
                                                <span className="text-[9px] leading-none mb-0.5 font-bold">{slab.rate !== null ? `${slab.rate}%` : '?%'}</span>
