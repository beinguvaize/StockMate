...
export const SalesProvider = ({ children }) => {
  ...
  const addClient = async (client) => {
    ...
    addNotification(`${client.name} added and synced`, "success");
  };
  ...
  const updateClient = async (updatedClient) => {
    ...
    if (isSupabaseConfigured) {
      if (error) {
        ...
        setSyncStatus('ERROR');
        addNotification(`Sync failure: ${error.message}`, "error");
        return; // STOP
      }
      setSyncStatus('SYNCED');
      setLastSyncedAt(new Date().toISOString());
  ...
  const recordClientPayment = async (
    ...
    if (isSupabaseConfigured) {
      ...
      if (clientError) {
        ...
        setSyncStatus('ERROR');
        addNotification(`Payment sync failed: ${clientError.message}`, "error");
    ...
    if (isSupabaseConfigured) {
      ...
      if (payErr) {
        ...
        addNotification(`History sync failed: ${payErr.message}`, "error");
  ...
  const updateSale = async (updatedSale) => {
    ...
    if (isSupabaseConfigured) {
      setSyncStatus('SYNCING');
      const { status: saleStatus, clientId, salesmanNote, items, subtotal, discount, tax, totalAmount, totalCogs, paymentMethod, paymentStatus, date, routeId, scheduledDate, bookedBy, paidAmount, lastPaymentDate, vehicleId, ...rest } = updatedSale;
      
      const dbSale = {
        ...rest,
        id: updatedSale.id,
        client_id: clientId || updatedSale.client_id || updatedSale.shopId,
        items,
        subtotal,
        discount,
        tax,
      ...
      if (error) {
        ...
        return; // STOP: Rollback local state if server failed
      }
      setSyncStatus('SYNCED');
      setLastSyncedAt(new Date().toISOString());
    }
    ...
    if (isSupabaseConfigured) {
      const { error } = await supabase
        ...
        .upsert({ 
          ...
          last_payment_date: updatedSale.lastPaymentDate,
          tenant_id: currentTenantId 
        });
      if (error) {
        ...
        addNotification(`Settlement failed: ${error.message}`, "error");
        return; // STOP
      }
    }
