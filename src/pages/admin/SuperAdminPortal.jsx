...
import { 
  ...
  Zap, Database, Globe, RefreshCcw, Mail, Settings, X, LogIn, ShieldAlert, Hash
} from 'lucide-react';
...
const SuperAdminPortal = () => {
  ...
  const fetchGlobalData = async () => {
    ...
    try {
      ...
      setGlobalStats(prev => ({
        ...
        activeUsers: tenantData?.reduce((acc, t) => acc + (t.staff_count || 1), 0) || 0
      }));
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-canvas text-ink-primary font-inter selection:bg-accent-signature/30 p-4 md:p-12 animate-in fade-in duration-700">
      {/* Nexus Header */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12 border-b border-black/5 pb-12">
        <div>
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-xs font-bold text-accent-signature uppercase tracking-[0.3em]">The Nexus Protocol</span>
          </div>
          <h1 className="text-6xl md:text-8xl font-black font-sora text-ink-primary leading-[0.85] tracking-tighter uppercase mb-4">
            CONSOLE<span className="text-accent-signature">.</span>
          </h1>
        ...
        <div className="space-y-6">
          ...
          <div className="glass-panel border-black/5 bg-white p-6 shadow-premium relative overflow-hidden">
            ...
            <div className="space-y-6">
              ...
              {recentActivities.map((activity, idx) => (
                <div key={activity.id + idx} className="flex gap-4 group cursor-default transition-all">
                  <div className={`shrink-0 w-8 h-8 rounded-lg bg-canvas border border-black/5 flex items-center justify-center ${activity.color} shadow-sm group-hover:scale-110 transition-transform`}>
                    <activity.icon size={14} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5">
                      <p className="text-[9px] font-black text-ink-primary uppercase tracking-tight truncate">
                        {activity.tenant?.name || 'System Override'}
                      </p>
            ...
            <div className="mt-8 pt-6 border-t border-black/5">
              <button className="w-full py-2 text-[9px] font-black text-accent-signature uppercase tracking-widest hover:tracking-[0.4em] transition-all">
                System Log Archive &rarr;
              </button>
            </div>
          </div>

          {/* System Notifications */}
          <div className="glass-panel border-black/5 bg-white p-6 shadow-sm">
            <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-6 border-b border-black/5 pb-4 flex items-center justify-between">
              Platform Alerts
              <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
            </h3>
            <div className="space-y-4">
              <div className="flex gap-4 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                <div className="text-emerald-600 shrink-0"><CheckCircle2 size={14} /></div>
                <div>
                  <p className="text-[10px] font-bold text-ink-primary uppercase mb-1">Backup Succeeded</p>
                  <p className="text-[9px] text-gray-500 font-medium whitespace-nowrap">Cross-region replication complete</p>
                </div>
              </div>
              <div className="flex gap-4 p-3 rounded-lg hover:bg-gray-50 transition-colors">
                <div className="text-amber-500 shrink-0"><AlertCircle size={14} /></div>
                <div>
                  <p className="text-[10px] font-bold text-ink-primary uppercase mb-1">Latency Spike Detected</p>
                  <p className="text-[9px] text-gray-500 font-medium whitespace-nowrap">AWS response delay &gt; 120ms</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col border-l border-black/5 animate-in slide-in-from-right duration-300">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input 
                      ...
                      ref={nameRef}
                      autoComplete="off"
                      className="w-full px-4 py-3 bg-canvas border border-black/5 rounded-xl text-xs font-bold focus:ring-2 focus:ring-accent-signature/20 focus:border-accent-signature outline-none transition-all"
                    />
                  </div>
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        ref={slugRef}
                        autoComplete="off"
                        className="w-full pl-7 pr-4 py-3 bg-canvas border border-black/5 rounded-xl text-xs font-mono font-bold focus:ring-2 focus:ring-accent-signature/20 focus:border-accent-signature outline-none transition-all uppercase"
                      />
                    </div>
                  </div>
                </div>
                  ...
                  {['STARTER', 'PRO', 'ENTERPRISE'].map(plan => (
                    <button
                      ...
                      className={`px-3 py-4 rounded-2xl border text-center transition-all ${
                        ...
                        ? 'bg-accent-signature text-white border-accent-signature shadow-xl shadow-accent-signature/20' 
                        : 'bg-white text-ink-primary/60 border-black/5 hover:border-black/20'
                      }`}
                    >
              ...
              <section>
                ...
                <div className={`flex items-center justify-between p-5 rounded-2xl border ${selectedTenant.status === 'SUSPENDED' ? 'bg-red-50 border-red-200' : 'bg-canvas border-black/5'}`}>
                  <div className="flex items-center">
                    ...
                    <span className="text-[10px] font-black text-ink-primary uppercase tracking-tight">{selectedTenant.status === 'SUSPENDED' ? 'Account Suspended' : 'Account Operational'}</span>
                  </div>
                  <button 
                    onClick={async () => {
                    ...
                    className={`text-[10px] font-black transition-colors px-4 py-2 rounded-xl border uppercase tracking-tighter ${
                      selectedTenant.status === 'SUSPENDED' 
                        ? 'text-emerald-600 hover:text-emerald-700 hover:bg-emerald-100 border-emerald-200' 
                        : 'text-red-600 hover:text-red-700 hover:bg-red-100 border-red-200'
                    }`}
                  >
                    {selectedTenant.status === 'SUSPENDED' ? 'Reactivate' : 'Suspend ID'}
                  </button>
                </div>
              </section>
            </div>
            ...
            <div className="p-6 border-t border-black/5 bg-canvas sticky bottom-0">
              <div className="flex space-x-3">
                <button 
                  onClick={() => setIsDrawerOpen(false)}
                  className="flex-1 py-3.5 bg-white border border-black/10 text-ink-primary text-[10px] font-black uppercase rounded-xl hover:bg-black/5 transition-all"
                >
                  Discard
                </button>
                <button 
                  onClick={handleSave}
                  disabled={isSaving}
                  className="flex-1 py-3.5 bg-ink-primary text-white text-[10px] font-black uppercase rounded-xl hover:opacity-90 transition-all shadow-2xl shadow-black/20 disabled:opacity-50"
                >
