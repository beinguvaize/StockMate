        ...
        <>
            <div className="animate-fade-in flex flex-col gap-4 pb-12">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 pb-4 border-b border-black/5">
                    <div>
                        <h1 className="text-3xl md:text-6xl font-black text-ink-primary uppercase tracking-tighter leading-none mb-2">PURCHASES.</h1>
                ...
                <div className="flex flex-col md:flex-row gap-4">
                    <div className="flex-1 relative group flex items-center !h-12">
                        ...
                        <input 
                            ...
                            className="w-full input-field !pl-12 !h-full !py-0 !rounded-pill bg-surface border border-black/5 shadow-premium text-sm font-bold placeholder:text-ink-secondary/30"
                            value={searchTerm}
                ...
                <div className="glass-panel !p-0 !rounded-bento border border-black/5 shadow-premium overflow-hidden">
                    <div className="bg-ink-primary p-4 flex items-center gap-4">
                        ...
                        <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-surface">Procurement Log</h2>
                    </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-black/5 bg-canvas font-inter">
                                    ...
                                    <th className="p-4 text-right text-[10px] font-black uppercase tracking-widest text-ink-secondary opacity-70 pr-8">Total Bill</th>
                                </tr>
                            </thead>
                                    ...
                                    return (
                                        <tr key={p.id} className="group hover:bg-canvas transition-colors">
                                            ...
                                            <td className="p-4">
                                                {product ? (
                                                    <div className="flex items-center gap-2">
                                                        <div className="text-xs font-black text-ink-primary uppercase tracking-tight">{product.name}</div>
                                                        <span className="px-1.5 py-0.5 rounded bg-black/5 text-[8px] font-black text-ink-secondary uppercase tracking-[0.2em]">{product.sku}</span>
                                                    </div>
                                                ) : (
                                                    <span className="px-2 py-1 rounded bg-black/5 text-[9px] font-black text-ink-secondary uppercase tracking-[0.2em] opacity-40 border border-black/5">Not Linked</span>
                                                )}
                                            </td>
                                            <td className="p-4 text-center">
                                                <div className="flex flex-col items-center">
                                                    <span className="px-3 py-1 rounded-pill bg-ink-primary/5 text-ink-primary text-[10px] font-black uppercase tracking-widest border border-black/5">
                                                        {p.quantity} {product?.unit || 'units'}
                                                    </span>
                                                    <div className="text-[9px] font-bold text-ink-secondary tracking-widest opacity-50 mt-1">@ {businessProfile?.currencySymbol || '₹'}{p.unit_cost?.toLocaleString()}</div>
                                                </div>
                                            </td>
                                            <td className="p-4 text-center">
                                                <div className="flex items-center justify-center gap-1.5">
                                                    ...
                                                    <span className={`text-[9px] font-black uppercase tracking-widest ${p.payment_type === 'cash' ? 'text-green-600' : 'text-orange-600'}`}>{p.payment_type || 'cash'}</span>
                                                </div>
                                            </td>
                                            <td className="p-4 text-right pr-8">
                                                ...
                                                <div className="text-[9px] font-black text-ink-secondary uppercase opacity-40">
                    ...
                    {filteredPurchases.length === 0 && (
                        <div className="p-32 text-center">
                            ...
                            <p className="text-sm font-black uppercase tracking-[0.4em] text-[#747576]">No procurement history</p>
                        </div>
                    )}
                </div>
            </div>
                ...
                <div className="modal-overlay">
                    <div className="glass-modal">
                        <div className="flex justify-between items-start mb-5">
                            <div>
                                <h2 className="text-3xl font-black text-ink-primary tracking-tighter uppercase leading-none mb-2">NEW PURCHASE.</h2>
                                <p className="text-[10px] font-black text-ink-secondary uppercase tracking-[0.3em] opacity-70">INCREASE STOCK VIA DIRECT PROCUREMENT</p>
                            </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="md:col-span-2">
                                    <label className="block text-[10px] font-black uppercase tracking-widest text-ink-secondary opacity-70 mb-1.5">Link to Product (Optional)</label>
                                    <select 
                                        className="w-full bg-canvas border-none rounded-2xl p-5 font-black text-xs text-ink-primary outline-none appearance-none cursor-pointer" 
                                ...
                                <div>
                                    <label className="block text-[10px] font-black uppercase tracking-widest text-ink-secondary opacity-70 mb-1.5">Batch Quantity</label>
                                    <input 
                                        required 
                                        type="number" 
                                        className="w-full bg-canvas border-none rounded-2xl p-5 font-black text-2xl text-ink-primary outline-none" 
                                ...
                                <div>
                                    <label className="block text-[10px] font-black uppercase tracking-widest text-ink-secondary opacity-70 mb-1.5">Unit Cost (CP)</label>
                                    <div className="relative">
                                        <div className="absolute left-6 top-1/2 -translate-y-1/2 text-xl font-black text-ink-primary opacity-20">
                                        ...
                                        <input 
                                            ...
                                            type="number" 
                                            step="0.01"
                                            className="w-full bg-canvas border-none rounded-2xl p-5 pl-12 font-black text-2xl text-ink-primary outline-none" 
                                ...
                                <div>
                                    <label className="block text-[10px] font-black uppercase tracking-widest text-ink-secondary opacity-70 mb-1.5">Purchase Date</label>
                                    <input 
                                        type="date" 
                                        className="w-full bg-canvas border-none rounded-2xl p-5 font-black text-xs text-ink-primary outline-none" 
                                        value={formData.date} 
                                ...
                                <div className="md:col-span-2 grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-[10px] font-black uppercase tracking-widest text-ink-secondary opacity-70 mb-1.5">Payment Type</label>
                                        <div className="flex gap-2 bg-canvas p-1 rounded-2xl border border-black/5">
                                            <button type="button" onClick={() => setFormData({...formData, payment_type: 'cash'})} className={`flex-1 py-3 px-4 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${formData.payment_type === 'cash' ? 'bg-white shadow-sm text-ink-primary' : 'text-ink-secondary opacity-50 hover:bg-black/5'}`}>Cash</button>
                                            <button type="button" onClick={() => setFormData({...formData, payment_type: 'credit'})} className={`flex-1 py-3 px-4 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${formData.payment_type === 'credit' ? 'bg-white shadow-sm text-ink-primary' : 'text-ink-secondary opacity-50 hover:bg-black/5'}`}>Credit</button>
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <input 
                                            type="text" 
                                            className="w-full h-[46px] bg-canvas border-none rounded-2xl px-5 font-black text-xs text-ink-primary outline-none placeholder:text-ink-secondary/30" 
