...
const Sales = () => {
 ...
 <h2 className="text-xl font-semibold text-ink-primary flex items-center gap-3">
 ...
 <div 
 className={`p-4 cursor-pointer border-b border-black/5 font-semibold text-[10px] hover:bg-canvas transition-colors ${selectedShopId === 'WALKIN' ? 'bg-accent-signature/20 text-ink-primary' : 'text-[#4b5563]'}`}
 onClick={() => {
 setSelectedShopId('WALKIN');
 setCustomerInfo({ name: 'Walk-in Customer', phone: ''});
 setCustomerSearch('');
 ...
 >
 <div className="font-semibold text-ink-primary text-sm">{s.name}</div>
 <div className="text-xs font-semibold text-[#4b5563] opacity-70 mt-1">{s.phone}</div>
 </div>
 ))}
 {filteredCustomers.length === 0 && (
 <div className="p-5 text-center text-xs font-semibold text-[#4b5563] opacity-70">No matches found</div>
 )}
 </div>
 </div>
 )}
 ...
 <input type="text" className="input-field !rounded-lg flex-1 text-xs !bg-surface" placeholder="NAME" value={customerInfo.name} onChange={e => setCustomerInfo({...customerInfo, name: e.target.value})} />
 <input type="text" className="input-field !rounded-lg flex-1 text-xs !bg-surface" placeholder="PHONE" value={customerInfo.phone} onChange={e => setCustomerInfo({...customerInfo, phone: e.target.value})} />
 </div>
 )}
 
 {selectedShopId !== 'WALKIN' && !showCustomerDropdown && (
 <div className="flex items-center gap-4 bg-ink-primary p-5 rounded-lg shadow-xl">
 ...
 <div className="text-sm font-semibold text-surface truncate">{customerInfo.name}</div>
 <div className="text-xs font-semibold text-surface/70 truncate">{customerInfo.phone}</div>
 </div>
 </div>
 )}
 </div>
 ...
 <div className="text-[11px] font-semibold text-ink-primary truncate leading-none mb-0.5">{item.name}</div>
 <div className="text-[9px] font-semibold text-[#4b5563] opacity-85">{item.sku}</div>
 </div>
 ...
 <div className="relative">
 <span className="text-[10px] font-semibold text-ink-primary opacity-85 absolute left-2.5 top-1/2 -translate-y-1/2">₹</span>
 <input 
 type="number" 
 className="w-full bg-canvas border border-black/5 rounded-full py-1 pl-5 pr-2 text-center text-[10px] font-semibold text-ink-primary outline-none focus:border-black/20 transition-all font-mono" 
 value={item.price} 
 ...
 <input 
 type="number" 
 className="w-10 text-center text-[10px] font-semibold text-ink-primary font-mono bg-transparent outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
 value={item.quantity}
 ...
 <span className="text-[10px] font-semibold -wider text-ink-primary">Total</span>
 ...
 <button 
 className="flex-1 py-3 rounded-pill border border-black/10 font-semibold text-ink-primary hover:bg-black/5 transition-all text-[9px] disabled:opacity-60 disabled:cursor-not-allowed" 
 ...
 <div className="text-sm font-semibold text-[#4b5563] mb-2 opacity-85">Total Amount</div>
 <div className="text-3xl md:text-6xl font-semibold text-ink-primary">
 ...
 {/* Header Section */}
 <div className="flex justify-between items-end pb-6 border-b border-black/5 mb-4">
 <div>
 <h1 className="text-4xl md:text-7xl font-black font-sora text-ink-primary leading-[0.85] tracking-tight mb-2 uppercase">SALES<span className="text-accent-signature">.</span></h1>
 <p className="text-[10px] font-semibold text-[#4b5563] opacity-[0.85]">Point of Sale & Transaction Flow</p>
 </div>
 </div>
 ...
 <select 
 className="input-field !pl-16 !py-2 !rounded-pill bg-surface border-black/5 shadow-premium appearance-none font-semibold text-[11px] cursor-pointer" 
 value={selectedRoute} 
 ...
 className={`px-6 py-2.5 rounded-pill text-[11px] font-semibold transition-all whitespace-nowrap flex items-center gap-3 ${
 ...
 <div className="text-[9px] font-semibold text-[#4b5563] mb-1 opacity-60 group-hover:opacity-100 transition-opacity">{product.sku}</div>
 <h3 className="text-base font-semibold text-ink-primary truncate leading-tight mb-1">{product.name}</h3>
 <div className={`inline-flex px-3 py-1 rounded-full text-[9px] font-semibold ${
 product.stock <= 10 ? 'bg-red-50 text-red-500' : 'bg-canvas text-[#4b5563] opacity-[0.85]'
}`}>
 ...
 <div className="text-[9px] font-semibold text-[#4b5563] mb-1 opacity-60">PRICE</div>
 <div className="text-2xl font-semibold text-ink-primary leading-none">
 ...
 <div className="w-10 h-10 rounded-full bg-ink-primary text-accent-signature flex items-center justify-center font-semibold text-sm shadow-2xl ring-4 ring-white animate-in zoom-in duration-300">
 {inCart.quantity}
 </div>
 </div>
 )}
 ...
 <p className="text-xs font-semibold mt-8">Empty Sector Search</p>
 </div>
 )}
 </div>
 </div>
 ...
 <div>
 <div className="text-xs font-semibold opacity-70">Cart Items</div>
 <div className="text-lg font-semibold">{cartCalc.totalItems} Items</div>
 </div>
 </div>
 <div className="text-right">
 <div className="text-xs font-semibold text-[#C8F135]">Total</div>
 <div className="text-2xl font-semibold">{businessProfile?.currencySymbol || '$'}{cartCalc.finalTotal.toLocaleString()}</div>
 </div>
 </div>
 )}
 ...
 <div>
 <h1 className="text-4xl md:text-7xl font-black font-sora text-ink-primary leading-[0.85] tracking-tight mb-2 uppercase">PAYMENT<span className="text-accent-signature">.</span></h1>
 <p className="text-[10px] font-semibold text-gray-600 opacity-80 mb-6 uppercase">SELECT TRANSACTION RECEPTION METHOD</p>
 </div>
 ...
 <div className="text-sm font-semibold text-gray-700 mb-2 opacity-70">Total Amount Due</div>
 <div className="text-3xl md:text-6xl font-semibold text-ink-primary">₹{cartCalc.finalTotal.toLocaleString()}</div>
 </div>

 {/* Itemized List (Compact) */}
 <div className="space-y-3">
 <div className="text-[10px] font-semibold text-gray-700 opacity-60 flex justify-between">
 ...
 <div className="text-[11px] font-semibold text-ink-primary truncate">{item.name}</div>
 <div className="text-[9px] font-semibold text-gray-700 opacity-70">{item.quantity} {item.unit} × ₹{item.price}</div>
 </div>
 <div className="text-sm font-semibold text-ink-primary">₹{item.lineTotal.toLocaleString()}</div>
 </div>
 ))}
 </div>
 </div>
 ...
 className={`p-4 rounded-lg border-2 transition-all flex items-center gap-4 relative group ${
 ...
 >
 <div className={`w-10 h-10 rounded-full flex items-center justify-center ${pendingPaymentMethod === method.id ? 'bg-ink-primary text-surface' : 'bg-canvas text-gray-700'}`}>
 <method.icon size={18} />
 </div>
 <span className="text-[10px] font-semibold leading-none text-left">{method.label}</span>
 </button>
 ))}
 </div>
 ...
 <button className="px-8 py-2 rounded-pill border border-black/10 font-semibold text-ink-primary text-xs hover:bg-black/5 transition-all cursor-pointer" onClick={() => setShowPaymentModal(false)}>Cancel</button>
 ...
 <div className="flex justify-between items-start mb-5">
 <div>
 <h1 className="text-4xl md:text-7xl font-black font-sora text-ink-primary leading-[0.85] tracking-tight mb-2 uppercase">PRE-BOOK<span className="text-accent-signature">.</span></h1>
 <p className="text-[10px] font-semibold text-gray-600 opacity-80 mb-6 uppercase">SET FUTURE DELIVERY SCHEDULE</p>
 </div>
 ...
 <div className="mb-5">
 <label className="block text-[10px] font-semibold text-gray-700 mb-1.5 opacity-60">Planned Delivery Date</label>
 <input 
 type="date" 
 className="w-full bg-canvas border-none rounded-lg p-6 font-semibold text-3xl text-center text-ink-primary outline-none focus:ring-8 focus:ring-accent-signature/20 transition-all" 
 value={scheduledDate} 
 ...
 <button className="px-8 py-2 rounded-pill border border-black/10 font-semibold text-ink-primary text-xs hover:bg-black/5 transition-all cursor-pointer" onClick={() => setShowBookingDateModal(false)}>Cancel</button>
