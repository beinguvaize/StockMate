-- Companion to 20260922_gl_journals_carry_the_transaction_date.sql: the two
-- sales posting functions, so a clean db reset reproduces production.
--
-- Both now pass the source transaction's own date to gl_journals instead of
-- letting it default to now(). See the companion migration for the numbers.
