...
const StatementTable = ({ sections = [], grandTotal, note }) => (
  <div className="bg-card rounded-[10px] border border-border/60 shadow-sm overflow-hidden">
    <table className="w-full">
      <thead>
        <tr className="border-b border-black/10 bg-canvas/40">
          <th className="px-6 py-3 text-left font-mono text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Particulars</th>
          <th className="px-6 py-3 text-right font-mono text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Amount</th>
        </tr>
      </thead>
      <tbody>
        {sections.map((sec, si) => {
              ...
              {(sec.groups || []).map((g, gi) => {
                ...
                return (
                  <React.Fragment key={gi}>
                    {g.title && (
                      <tr>
                        <td className="px-6 pt-2.5 pb-1 text-[11px] font-bold uppercase tracking-wide text-muted-foreground" colSpan={2}>{g.title}</td>
                      </tr>
                    )}
                    {g.lines.map((l, li) => (
                    ...
                    <tr className="border-t border-black/[0.06]">
                      <td className="pl-10 pr-6 py-1.5 text-[12px] font-bold text-muted-foreground uppercase tracking-wide">{g.subtotalLabel || `Total ${g.title || ''}`.trim()}</td>
    ...
    {note && <div className="px-6 py-3 border-t border-black/5 text-[10px] font-medium text-muted-foreground">{note}</div>}
  </div>
);
