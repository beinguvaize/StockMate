...
const GeneralLedgerReport = () => {
  ...
  const renderTypeBadge = (val) => {
    const colors = {
      ...
      PAYROLL: 'bg-violet-50 text-violet-600',
  ...
  const journalTab = {
    ...
    columns: [
      ...
      { key: 'debit', label: 'Debit', type: 'currency', align: 'right', sortable: true, width: 150, render: (val) => val > 0 ? <span className="font-black text-indigo-600">{formatINR(val)}</span> : <span className="text-gray-300">—</span> },
    ...
    chartConfig: {
      ...
      series: [
        { key: 'Debits', name: 'Debits', color: '#6366f1' },
  ...
  const accountTab = {
    ...
    columns: [
      ...
      { key: 'debit', label: 'Total Debits', type: 'currency', align: 'right', sortable: true, width: 160, render: (val) => <span className="font-black text-indigo-600">{formatINR(val)}</span> },
    ...
    chartConfig: {
      ...
      series: [{ key: 'value', name: 'Balance', color: '#6366f1' }],
    },
    detailFields: [
