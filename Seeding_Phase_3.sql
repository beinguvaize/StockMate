...
BEGIN
    ...
    FOR i IN 0..13 LOOP
        ...
        INSERT INTO public.sales (id, "shopId", "paymentMethod", "paymentStatus", "totalAmount", date, status, payment_type, is_seed, tenant_id)
        VALUES (
            sale_id, cli_id, 'CASH', 'PAID', 
            floor(random() * 3000 + 2000), 
            d::text, 'COMPLETED', 'cash', TRUE, v_tenant_id
        ...
        VALUES (
            ...
            d::text, 'COMPLETED', 'credit', TRUE, v_tenant_id
        ) ON CONFLICT (id) DO NOTHING;

        -- Daily Expense to show in Trend Chart
        INSERT INTO public.expenses (id, category, amount, note, date, is_seed, tenant_id)
        VALUES (
            'seed_trend_exp_' || i,
            'Fuel', 
            floor(random() * 500 + 300), 
            'Daily operational fuel', 
            d::text, TRUE, v_tenant_id
        ) ON CONFLICT (id) DO NOTHING;
    END LOOP;
    ...
    FOR i IN 1..20 LOOP
        ...
        INSERT INTO public.movement_log (id, date, product_id, product_name, type, quantity, reason, is_seed, tenant_id)
        VALUES (
            'seed_mov_out_' || i,
