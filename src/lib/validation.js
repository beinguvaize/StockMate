...
// Backend stores `unit_cost` (snake_case); legacy form callers may pass `unitCost`.
// Preprocess to normalize either key to `unit_cost` before validation.
export const purchaseSchema = z.preprocess(
  (data) => {
    if (data && typeof data === 'object' && !('unit_cost' in data) && 'unitCost' in data) {
      return { ...data, unit_cost: data.unitCost };
    }
    return data;
  },
  z.object({
    quantity: z.number().positive('Purchase quantity must be positive').or(z.string().regex(/^\d+$/).transform(Number)),
    unit_cost: z.number().min(0, 'Unit cost cannot be negative').or(z.string().regex(/^\d+(\.\d+)?$/).transform(Number)),
